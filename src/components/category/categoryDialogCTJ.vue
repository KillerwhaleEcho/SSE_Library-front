<!-- CategoryDialogCTJ.vue -->
<template>
    <el-dialog :model-value="visible" @update:model-value="$emit('update:visible', $event)" title="选择分类" :modal="false"
        append-to-body :z-index="2000">
        <div class="search-cat">
            <el-input v-model="searchCatKeyword" placeholder="搜索分类" class="search-input" append-to-body
            ...
            <div>
                <el-popover class="box-item" placement="right-start" width="350" trigger="click"
                    v-model:visible="popoverVisible">
                    <!-- 添加分类表单内容 -->
                    <el-form ref="categoryFormRef" :model="categoryFormData" label-width="50px" size="small"
                        class="add-form">
                        <!-- 1. 类型选择（分类/课程） -->
                        <el-form-item label="类型" prop="isCourse"
                        ...
                        <el-form-item label="名称" prop="name"
                            ...
                            <el-input v-model="categoryFormData.name" placeholder="请输入名称" maxlength="50" />
                        </el-form-item>

                        <!-- 2.2 所属分类（选择"课程"时显示） -->
                        <el-form-item v-if="categoryFormData.isCourse === true" label="所属分类" prop="parentCatId"
                            :rules="[{ required: true, message: '请选择所属分类', trigger: 'change' }]">
                            <el-select v-model="categoryFormData.parentCatId" placeholder="请选择分类" clearable
                                :teleported="false" :popper-append-to-body="false">
                                <el-option v-for="category in allCategories" :key="category.id" :label="category.name"
                                    :value="category.id" />
                            </el-select>
                        </el-form-item>
                        ...
                        <el-form-item label="描述" prop="description">
                            <el-input v-model="categoryFormData.description" placeholder="请输入描述信息（可选）" type="textarea"
                        ...
                        <el-form-item style="margin-bottom: 0;">
                            <div style="display: flex; justify-content: flex-end; gap: 10px;">
                                <el-button class="cancel-button" size="small" @click="resetCategoryForm">
                                    取消
                                </el-button>
                                <el-button class="reset-button" size="small" type="primary" @click="submitCategoryForm">
                                    确认添加
                                </el-button>
                            </div>
                        </el-form-item>
        ...
        <div class="category-dialog">
            <div v-for="category in allCategories" :key="category.id" class="category-row">
                <div class="parent-item">
                    <CategoryClickToJump :category="category" @click="onCategorySelected" />
                </div>
                <div class="child-items" v-if="category.children && category.children.length">
                    <CategoryClickToJump v-for="subCategory in category.children" :key="subCategory.id"
                        :category="subCategory" @click="onCategorySelected" />
                </div>
            </div>
        </div>
        <template #footer>
