    ...
    <div class="page-body">
            ...
            <div v-if="documentDetail" class="document-hero">
              ...
              <div class="hero-info">
                ...
                <div class="meta-grid" v-if="baseDetails.length">
                  <div
                    v-for="item in baseDetails"
                    :key="item.label"
                    :class="[
                      'meta-item',
                      { clickable: item.key === 'category' && item.value },
                    ]"
                    @click="
                      item.key === 'category' && item.value
            ...
            <div v-if="documentDetail" class="document-actions-card">
              ...
              <div class="document-actions-row">
                ...
                <button
                  ...
                  :class="{ disabled: !canPreview || previewLoading }"
              ...
              >
                <button
                  class="doc-action-button admin"
                  :class="{ disabled: statusUpdating }"
                  :disabled="statusUpdating"
                  @click="handleStatusUpdate('open')"
                >
                  <span>开放 open</span>
                </button>
                <button
              ...
              <div
                v-else-if="isUploaderViewer"
                class="document-actions-row admin-action-row"
              >
                <button
                  class="doc-action-button admin"
                  :class="{ disabled: withdrawProcessing }"
      ...
      <section class="discussion-section">
        <div class="discussion-header">
          ...
          <div class="discussion-tabs">
            <button
              ...
              :class="{ active: activeDiscussionTab === 'comment' }"
              @click="setDiscussionTab('comment')"
            >
              评论
            </button>
            ...
            <button
              type="button"
              class="discussion-tab"
              :class="{ active: activeDiscussionTab === 'post' }"
        ...
        <div v-if="activeDiscussionTab === 'comment'" class="discussion-panel">
          <CommentSection
            v-if="documentDetail && documentNumericId !== null"
            source-type="document"
            :source-id="documentNumericId"
            :source-data="documentSourceData"
...
const canPreview = computed(
  () => brief.value?.type === "book" && Boolean(documentDetail.value?.URL)
);
...
const uploaderUserId = computed<number | null>(
  () => documentDetail.value?.uploader?.userId ?? null
);
const isUploaderViewer = computed(() => {
  const uploaderId = uploaderUserId.value;
  if (uploaderId === null || uploaderId === undefined) return false;
  const localId =
    localUserIdFromStorage.value ?? userInfo.value?.userId ?? null;
  if (localId === null || localId === undefined) return false;
  return Number(localId) === Number(uploaderId);
});
const documentPosts = computed<Post[]>(
  () => documentDetail.value?.postList ?? []
);

const favoriteLabel = computed(() =>
  favoriteJudgement.value ? "停止收藏" : "收藏"
);
const favoriteIconSrc = computed(() =>
  favoriteJudgement.value ? unlikeIcon : likeIcon
);
const isVideoDocument = computed(() => brief.value?.type === "video");
const downloadButtonLabel = computed(() =>
  isVideoDocument.value ? "查看" : "下载"
);
const downloadButtonIcon = computed(() =>
  isVideoDocument.value ? previewOpenIcon : downloadIcon
);
...
const currentDocumentStatus = computed(() =>
  brief.value?.status
    ? docStatusDisplayMap[brief.value.status] ?? brief.value.status
    : "未知"
);
...
const handleFavorite = async () => {
  ...
  try {
    ...
    ElMessage.error(
      error?.message || (wasFavorited ? "取消收藏失败" : "收藏失败")
    );
  } finally {
...
const handleStatusUpdate = async (targetStatus: "open" | "closed") => {
  ...
  try {
    await updateDocumentStatus({
      documentId: Number(docNumericId),
      status: targetStatus,
    });
...
watch(
  [() => userInfo.value?.userId, () => documentNumericId.value],
  async () => {
    await refreshFavoriteJudgement();
  },
  { immediate: true }
...
watch(
  ...
  (newVal, oldVal) => {
    ...
    if (newVal) {
      console.log("文件详情页上传模态框已打开");
    } else {
      console.log("文件详情页上传模态框已关闭");
    }
  }
);
